import json
import os
import pygmo as pg
import numpy as np
import client
import ServerTorcs
import clientThread
from def_param import used_parameters, believe, w
import matplotlib.pyplot as plt
import threading


class myProblem:

    def fitness(self, dv):
        i = 0
        P = used_parameters
        for key in P:
            P[key] = dv[i]
            i = i + 1

        server = ServerTorcs.ServerTorcs()
        server.start()
        print "client 1 start"
        (lapTimes, damages) = client.main(P, 3001)
        print "client 1 stop"
        # server1 = ServerTorcs.ServerTorcs1()
        # server1.start()
        # print "client 2 start"
        # (lapTimes1, damages1) = client.main(P, 3002)
        # print "client 2 stop"
        # server2 = ServerTorcs.ServerTorcs1()
        # server2.start()
        # print "client 3 start"
        # (lapTimes2, damages2) = client.main(P, 3003)
        # print "client 3 stop"

        print "\n", P

        print "lastLapTime--->", lapTimes[1]  # , '--', lapTimes1[1], '--', lapTimes2[1],

        damagePerLap = damages[1] - damages[0]  # self.getDamagePerLap(damages)
        # damagePerLap1 = damages1[1] - damages1[0]
        # damagePerLap2 = damages2[1] - damages2[0]
        print "damages--->", damagePerLap  # , '--', damagePerLap1, '--', damagePerLap2

        fitness = lapTimes[
                      1] + damagePerLap / 2  # + lapTimes1[1] + damagePerLap1 / 2 + lapTimes2[1] + damagePerLap2 / 2
        print "fitness--->", fitness
        return [fitness]

    def get_bounds(self):
        y = used_parameters
        LOWER_VECTOR = []
        UPPER_VECTOR = []
        percent = 0.2
        for elem in y:
            if y[elem] > 0:
                UPPER_VECTOR.append(y[elem] + y[elem] * percent)
                LOWER_VECTOR.append(y[elem] - y[elem] * percent)
            if y[elem] < 0:
                UPPER_VECTOR.append(y[elem] - y[elem] * percent)
                LOWER_VECTOR.append(y[elem] + y[elem] * percent)
            if y[elem] == 0:
                UPPER_VECTOR.append(10)
                LOWER_VECTOR.append(-10)
        return (LOWER_VECTOR, UPPER_VECTOR)


def mySADE(n_trials, n_gen, p_size, new_parameters, in_pop=None):
    prob = pg.problem(myProblem())
    print "Problem defined!"
    uda = pg.sade(gen=n_gen, variant=4, variant_adptv=1, memory=False, seed=1234, ftol=1e-6, xtol=1e-6)

    global_results = []
    logs = []
    algo = pg.algorithm(uda)
    results_trial = []
    for i in range(0, n_trials):
        log_trial = []
        algo.set_verbosity(9)
        if in_pop:
            pop = in_pop
        else:
            pop = pg.population(prob, p_size)
        algo.evolve(pop)
        log_trial.append(algo.extract(type(uda)).get_log())
        log_trial = np.array(log_trial)
        results_trial.append(np.min(log_trial[:, log_trial.shape[1] - 1, 2]))
    logs.append(algo.extract(type(uda)).get_log())
    logs = np.array(logs)
    avg_log = np.average(logs, 0)
    global_results.append(np.min(results_trial, 0))
    plt.plot(avg_log[:, 1], avg_log[:, 2], label=algo.get_name())

    print("global results: ", global_results)

    P = used_parameters
    i = 0
    for key in P:
        P[key] = pop.champion_x[i]
        i = i + 1
    os.chdir(r"C:\Users\alex\Documents\GitHub\ComputazioneNaturale\snakeoil2015")
    print "champion_x", pop.champion_x
    print "champion_f", pop.champion_f
    with open("def_param.py", 'a') as outfile:
        outfile.write(new_parameters + " = " + str(P))
        json.dump(P, outfile)
    with open(new_parameters + ".txt", 'w') as outfile:
        json.dump(P, outfile)
    # We then add details to the plot
    plt.legend()
    plt.grid()
    plt.show()


# def mySADE_multipleUDA(n_trials, n_gen, p_size, new_parameters, in_pop=None):
#     prob = pg.problem(myProblem())
#     print "Problem defined!"
#     udas = [
#         pg.sade(gen=n_gen, variant=4, variant_adptv=1, memory=False, seed=1234, ftol=1e-6, xtol=1e-6),
#         pg.sade(gen=n_gen, variant=5, variant_adptv=1, memory=False, seed=1234, ftol=1e-6, xtol=1e-6),
#         pg.sade(gen=n_gen, variant=6, variant_adptv=1, memory=False, seed=1234, ftol=1e-6, xtol=1e-6),
#         pg.sade(gen=n_gen, variant=7, variant_adptv=1, memory=False, seed=1234, ftol=1e-6, xtol=1e-6)
#     ]
#
#     global_results = []
#     for uda in udas:
#         logs = []
#         algo = pg.algorithm(udas)
#         results_trial = []
#         for i in range(0, n_trials):
#             log_trial = []
#             algo.set_verbosity(9)
#             if in_pop:
#                 pop = in_pop
#             else:
#                 pop = pg.population(prob, p_size)
#             algo.evolve(pop)
#             log_trial.append(algo.extract(type(uda)).get_log())
#             log_trial = np.array(log_trial)
#             results_trial.append(np.min(log_trial[:, log_trial.shape[1] - 1, 2]))
#         logs.append(algo.extract(type(uda)).get_log())
#         logs = np.array(logs)
#         avg_log = np.average(logs, 0)
#         global_results.append(np.min(results_trial, 0))
#         plt.plot(avg_log[:, 1], avg_log[:, 2], label=algo.get_name())
#
#     print("global results: ", global_results)
#
#     P = used_parameters
#     i = 0
#     for key in P:
#         P[key] = pop.champion_x[i]
#         i = i + 1
#     os.chdir(r"C:\Users\alex\Documents\GitHub\ComputazioneNaturale\snakeoil2015")
#
#     with open("def_param.py", 'a') as outfile:
#         outfile.write(new_parameters + " = " + str(P))
#         json.dump(P, outfile)
#     with open(new_parameters + ".txt", 'w') as outfile:
#         json.dump(P, outfile)
#     # We then add details to the plot
#     plt.legend()
#     plt.grid()
#     plt.show()


if __name__ == "__main__":
    n_trials = 1
    population_size = 10
    n_gens = 200
    pg.set_global_rng_seed(seed=42)
    new_parameters = "Test30"
    mySADE(n_trials, n_gens, population_size, new_parameters)
